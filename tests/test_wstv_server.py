...
def test_output_filename_is_sanitized_and_kept_in_downloads():
    ...
    assert path.parent == wstv_server.DOWNLOADS_DIR


def test_output_filename_rejects_path_traversal():
...
def test_run_pipeline_request_sanitizes_logs_and_history(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(wstv_server, "DOWNLOADS_DIR", tmp_path / "downloads")
