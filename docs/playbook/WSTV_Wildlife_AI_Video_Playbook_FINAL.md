...
**Final PDF Edition**
Wild Stories TV production system for vertical wildlife AI videos
Prepared for local review and PDF distribution

---

Step-by-step AI production system for Wild Stories TV wildlife videos.

**Production version:** Full playbook — Chapters 1–19 + Appendices A–F
**Status:** Final PDF-ready source
...
# WSTV Quick-Start Card

Use this one-page card at the start of each daily production.

## 7-Step Daily Workflow

1. Upload or open this playbook in AI.
2. Paste the universal starter prompt.
3. Answer: primary animal, scene category, habitat.
4. Follow chapters 3 → 16 in order.
5. Pass every gate before moving forward.
6. Keep final Seedance prompt under 3500 characters.
7. Dry-run before paid generation; paid confirmation is exactly `SUBMIT_ONE_PAID_TASK`.

## Top 3 Mistakes

- Jumping to final Seedance prompt before locking idea/realism/master/storyboard.
- Treating REVISE as PASS.
- Logging dry-runs as paid usage.

## Data Safety Reminder

Never paste API keys, signed URLs, private JSON, MP4 links, or `data/` contents into AI.
...
| WSTV Quick-Start Card | One-page daily workflow and data-safety reminder |
...
Do not generate the final video prompt from this example yet. The gates must pass first because final video prompts depend on locked idea, realism, master image, and storyboard.
...
Output exactly this 12-column table:
1. #
2. Title
3. Animal(s)
4. Habitat
...
Reference Image 2 is a storyboard/motion guide only. Do not copy grid lines, borders, frame numbers, panel captions, text, logos, watermarks, or storyboard layout into the video. Use it only for shot order, framing, pacing, and motion continuity.
...
- If all boxes pass: go to Chapter 9 and run the local dashboard dry run with the approved final prompt.
...
- Resolution: 720p for testing. Use 1080p only for final/high-value scenes.
- no blood, gore, visible injury, bite contact, animal death, harmed baby animal, or unsafe human contact
- no humans unless explicitly requested
...
## If Dashboard or BytePlus Is Unavailable

- Do not submit a second paid task blindly.
- Check local dashboard status/logs.
- Check BytePlus Console status/billing if relevant.
- Keep Safe Mode ON until the issue is understood.
- Archive the failed/blocked attempt as “service unavailable”.
- Retry later only after dry-run and submit gates are clear.
...
Rules:
- Safety note: Do not give unsafe wildlife handling advice. Do not claim real footage if AI-generated. Be honest, brief, and calm if asked about AI.
...
BytePlus Console Billing remains the final source of truth. The dashboard/local ledger is for creator tracking only. This is a tracking example, not a future price guarantee. Always trust current BytePlus Console billing.
...
## WSTV DATA SAFETY RULES

- Do not paste API keys into AI.
- Do not paste signed URLs into AI.
- Do not paste private task JSON into AI.
- Do not paste MP4 links or generated video files into AI unless explicitly needed for review and safe.
- Do not commit `.env`, `data/`, `outputs/`, private responses, or dashboard-generated files.
- Save only safe production notes, prompts, verdicts, and public-facing social copy.
...
CAUTION: Review this note before saving. Remove API keys, signed URLs, MP4 links, private task JSON, and any secrets before storing anywhere.
...
| Storyboard grid copied | `Reference Image 2 is a storyboard/motion guide only. Do not copy grid lines, borders, frame numbers, panel captions, text, logos, watermarks, or storyboard layout into the video. Use it only for shot order, framing, pacing, and motion continuity.` |
...
| CapCut Export Checklist | Before final export accepted | Social pack |
...
BytePlus Console Billing remains the final source of truth. The local ledger is creator tracking only. This is a tracking example, not a future price guarantee. Always trust current BytePlus Console billing.
...
END OF WSTV WILDLIFE AI VIDEO PLAYBOOK FINAL
